				...
				<div className="shopping-list-recipes-section">
					...
					<ul>
						{recipeEntries.map((entry) => (
							<li key={entry.id}>
								<span>{entry.recipeName} — {entry.servings} {recipeServingsLabels[entry.recipeName] ?? t('shoppingListDisplay.recipesSection.servingsFallback')}</span>
										...
										<span className="shopping-list-item-text">
											{isEditing ? (
												<input
													...
													placeholder={t('shoppingListDisplay.owned.placeholder')}
												...
												<span
													...
													title={t('shoppingListDisplay.owned.title')}
												>
													{resolved.item.name}
												</span>
											)}
											{resolved.item.complement && ` (${resolved.item.complement})`}
											{formatAggregation(resolved, t) && ` — ${formatAggregation(resolved, t)}`}
											{resolved.aggregation.ownedSubtractionFailed && (
												<span className="ingredient-validation-error"> {t('shoppingListDisplay.owned.incompatibleUnit')}</span>
											)}
										</span>
